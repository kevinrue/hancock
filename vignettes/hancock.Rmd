...
```{r}
levels(tgs$set) <- gsub("_", " ", levels(tgs$set))
table(tgs$set)
```

## Interactively

In addition, the `r Githubpkg("kevinrue/hancock")` package includes a lightweight `r CRANpkg("shiny")` app that offers users the possibility to interactively rename signatures while inspecting their features in an `SummarizedExperiment` object (e.g., count of samples associated with each signature, distribution in reduced dimension).
...
if (interactive()) {
    library(shiny)
    allen.hancock <- runApp(shinyLabels(tgs, allen.hancock))
}
```
...
As described in the accompanying vignette "concepts.Rmd", absolute markers may be defined as genes detected in each cluster, irrespective of their expression in the other clusters.
...
As the numbers of genes detected in each cluster may be rather large, it is generally a good idea to restrict markers to be detected in a high fraction of the corresponding cluster, for instance `min.prop = 0.9`.
In addition, the `threshold` argument may be used to define a minimal threshold of expression level 
...
```{r}
tgs <- learnSignatures(
    se = allen, assay.type = "cufflinks_fpkm", method = "PositiveProportionDifference",
    cluster.col = "driver_1_s", min.diff = 0, min.prop = 0.9, threshold = 0)
table(tgs$set)
```

## Relative markers

As described in the accompanying vignette "concepts.Rmd", relative markers may be defined by _differential_ analysis against other cells in the same sample.
