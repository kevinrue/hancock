...
Those will be used below to learn signatures associated with well characterized populations cell.
...
Furthermore, we also require the combined set of markers to be detected in at least 10% of the target cluster (`min.prop = 0.1`).
...
```{r}
mcols(elementData(basesets)) <- cbind(
    mcols(elementData(basesets)),
    rowData(tenx_pbmc3k)[
        ids(elementData(basesets)),
...
Similarly, metadata associated with each unique signature--irrespective of its specific relationships with individual markers--are stored in the metadata columns of the `setData` slot.
...
```{r}
ids(setData(basesets)) <- paste0(seq_along(setData(basesets)), ". ", ids(setData(basesets)))
ids(setData(basesets))
```
...
assay(tenx_pbmc3k, "logcounts") <- as(assay(tenx_pbmc3k, "logcounts"), "Matrix")
tenx_pbmc3k <- runPCA(tenx_pbmc3k, method="irlba")
